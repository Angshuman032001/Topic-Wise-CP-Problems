//https://www.codechef.com/SPY22022/problems/CP_04
...
void solve(){
    ...
    function<void(int,int)>dfs = [&](int node, int par)->void{
        ...
        for(auto child : adj[node]){
            if(child.first == par) continue;
            dp[node][0] = max(dp[node][0], child.second + dp[child.first][1]);
            dp[node][1] = max(dp[node][1], dp[child.first][0]);
        }
        ...
        for(auto child : adj[node]){
            if(child.first == par) continue;
            store.push_back({child.first, dp[child.first][0]});
        }

        sort(all(store), [&](auto x, auto y)->bool{
            return x.second < y.second;
        });

        for(auto child : adj[node]){
            if(child.first == par) continue;
            int firstValue = child.second + dp[child.first][1];
            int addedValue = 0;
            if(!store.empty() && store.back().first != child.first) addedValue = store.back().second;
            else if(sz(store) > 1 && store[sz(store) - 2].first != child.first) addedValue = store[sz(store) - 2].second;
            ans = max(ans, firstValue + addedValue);
        }
    };
